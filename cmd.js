	    ...
	    return ({
	        ...
	        input: {
	            ...
	            TAB: function (self) {
	                ...
	                var temp = '';
	                var DATA = '';
	                if (self.buffer.length != 0) {//not at prompt
	                    for (var i = 0; i < len; i++) {
	                        if (keys[i].indexOf(self.buffer.split(":").pop()) == 0
                            && obj[keys[i]] !== null
                            && typeof (obj[keys[i]]) == "object"
                            && obj[keys[i]].NameSpace.replace('.' + keys[i], '') == self.cmd) {//LOCAL NAMESPACE SEARCH
	                            count++;
	                            temp = keys[i];
	                        }
	                    }
	                    if (count == 1) {
	                        ...
	                        tmp.pop();
	                        (tmp.length > 0) ? temp = ":" + temp : temp = temp;
	                        self.buffer = tmp.join(":") + temp;
	                        ...
	                        for (var i = 0; i < len; i++) {
	                            if (keys[i].indexOf(self.buffer.split(":").pop()) == 0
                                    && obj[keys[i]] !== null
                                    && typeof (obj[keys[i]]) == "object") {//GLOBAL NAMESPACE SEARCH
	                                DATA += '\r\n  ' + keys[i];
	                                temp = keys[i];
	                                count++;
	                            }
