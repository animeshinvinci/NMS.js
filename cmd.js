            ...
            input: {
                ...
                QUESTION: function (self) {
                    ...
                    if (self.buffer.length == 0) {// at prompt
                        ...
                        for (var i = 0; i < len; i++) {
                            if (keys[i].indexOf(self.buffer) == 0 && obj[keys[i]] !== null && typeof (obj[keys[i]]) == "object") {
                                var desc = obj[keys[i]].DESCRIPTION ? obj[keys[i]].DESCRIPTION.replace(/  /g, '').replace(/\r/g, ' ').replace(/\n/g, '').wrap(70, '\r\n\t\t\t\t') : 'No description avaiable';
                                DATA += '\r\n   ' + keys[i].max(25) + '\t' + desc;
                            }
                        }
                ...
                TAB: function (self) {
                    ...
                    if (self.buffer.length != 0) {//not at prompt
                        for (var i = 0; i < len; i++) {
                            if (keys[i].indexOf(self.buffer) == 0
                            && obj[keys[i]] !== null
                            && typeof (obj[keys[i]]) == "object"
                            && obj[keys[i]].NameSpace.replace('.' + keys[i], '') == self.cmd) {
                                count++;
                                temp = keys[i];
                            }
                        }
